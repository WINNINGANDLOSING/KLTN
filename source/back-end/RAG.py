...
from prompts import gen_qa_prompt, gen_rag_answer, gen_rag_answer2, formatted_context, intent_classification_prompt, text_summarization_prompt
...
class RAG:
    ...
    def response(self, query_str):
        ...
        response = self.model.complete(
            gen_rag_answer2.format(query_str=query_str, context_str=context)
        )
...
def format_sources(sources):
    ...
    for key, value in sources:
        if key in combined_dict:
            # If the key already exists, append the new value with the separator
            combined_dict[key] += f"       ================= --> ================== \n{value}"
        else:
    ...
    result = [[key, combined_dict[key]] for key in combined_dict]
    
    docs = [v[0] for v in result]
    context = [v[1] for v in result]
